...
class ReportController
{
    ...
    public function saveSettings(): void
    {
       ...
       $allowed = [
    'site_name', 'store_logo', 'store_description', 
    'contact_email', 'contact_phone', 'address',    
    'shipping_fee', 'free_ship_threshold',          
    'send_order_email',
];
...
// --- THÊM ĐOẠN NÀY VÀO DƯỚI CÙNG CỦA FILE (Ngoài Class) ---

// 1. Khởi tạo (gọi) Class
$reportController = new ReportController();

// 2. Dựa vào URL đang truy cập để bấm nút chạy đúng hàm
$uri = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);

if (strpos($uri, '/admin/reports/export') !== false) {
    // Nếu URL có chữ export -> Chạy hàm xuất file Excel
    $reportController->exportCSV();
} elseif (strpos($uri, '/admin/reports') !== false) {
    // Nếu URL chỉ là reports -> Chạy hàm hiển thị giao diện báo cáo
